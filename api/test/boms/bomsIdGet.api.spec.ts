...
describe('{API] boms/{id} endpoint GET', () => {
    ...
    it("should be an error for non existed bom id", async () => {
        ...
        const nonExitedBomById:any = await Boms.getBomById(user.userAdmin,nonExistedID);
    ...
    it("should be an error for non valid bom id", async () => {
        ...
        const nonExitedBomById:any = await Boms.getBomById(user.userAdmin,notValidID);
