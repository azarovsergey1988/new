...
describe('{API] boms/{id}/children endpoint POST', () => {
    ...
    it("should show BOM children by id if existed", async () => {
        ...
        const bomChildren: any = await Boms.getBomChildrenByIdWIthPostMethod(user.userAdmin, assemblyBoms[0].id);
    ...
    it("should be an error for non existed bom id", async () => {
        ...
        const bomChildrenNonExisted: any = await Boms.getBomChildrenByIdWIthPostMethod(user.userAdmin, nonExistedID);
    ...
    it("should be an error for non valid bom id", async () => {
        ...
        const bomChildrenNonExisted: any = await Boms.getBomChildrenByIdWIthPostMethod(user.userAdmin, notValidID);
