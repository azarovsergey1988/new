...
describe('Parametric Search  - SC Risk column', () => {
    beforeAll(async () => {
    ...
    it('should be sort options', async () => {
        await grid.newGridOpenFilterBoxByName(columnHeaders.search.parametric[3]);
    ...
    it('should work Autosize this column option', async () => {
        ...
        await expect(await JsScripts.returnElementWidthByCssAndNumber(gridElements.newGridHeaderCss, 4))
            .toBeCloseTo(106);
        await grid.newGridOpenFilterBoxByName(columnHeaders.search.parametric[3]);
    ...
    it('should work Reset columns', async () => {
        const defaultWidth: number = await JsScripts.returnElementWidthByCssAndNumber(gridElements.newGridHeaderCss, 4);
        await grid.newGridOpenFilterBoxByName(columnHeaders.search.parametric[3]);
        await grid.selectOptionInColumnFilter(columnHeaders.bom.sortOptions2[1]);
        await grid.newGridOpenFilterBoxByName(columnHeaders.search.parametric[3]);
        await grid.selectOptionInColumnFilter(columnHeaders.bom.sortOptions2[3]);
    ...
    it('should has Hide and Unhide ability', async () => {
        await grid.newGridHideColumnByName(columnHeaders.search.parametric[3]);
        await toolbar.unhideCellNameWithUnhideAll(columnHeaders.search.parametric[3]);
    });
})