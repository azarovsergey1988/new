...
describe('Parametric Search  - ENV Risk column', () => {
    ...
    it('should has default width', async () => {
        await expect(await JsScripts.returnElementWidthByCssAndNumber(gridElements.newGridHeaderCss, 3))
            .toBeCloseTo(79,-1);
    });

    it('should be sort options', async () => {
        await grid.newGridOpenFilterBoxByName(columnHeaders.search.parametric[2]);
    ...
    it('should work Reset columns', async () => {
        const defaultWidth: number = await JsScripts.returnElementWidthByCssAndNumber(gridElements.newGridHeaderCss, 3);
        await grid.newGridOpenFilterBoxByName(columnHeaders.search.parametric[2]);
        await grid.selectOptionInColumnFilter(columnHeaders.bom.sortOptions2[1]);
        await grid.newGridOpenFilterBoxByName(columnHeaders.search.parametric[2]);
        await grid.selectOptionInColumnFilter(columnHeaders.bom.sortOptions2[3]);
    ...
    it('should has Hide and Unhide ability', async () => {
        await grid.newGridHideColumnByName(columnHeaders.search.parametric[2]);
        await toolbar.unhideCellNameWithUnhideAll(columnHeaders.search.parametric[2]);
    });
})