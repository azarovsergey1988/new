...
describe('Parts Search Transpose', () => {
    ...
    it('should add bom to BOMs from transpose', async () => {
        await transpose.transposeCheckboxRangeChecking(0, 1);
        await toolbar.openToolbarDropdownByButtonName(buttonNames.add);
        await button.clickByButtonName(buttonNames.addToBoms);
        await modal.checkModalTitleName(modalTitles.addSelectedPartsToBom, 0);
    });

    it('should add bom to workspace from transpose', async () => {
        await login.loginWithDirectLink(browser.params.groupAdminUrl);
        ...
        await button.clickOnTheElementAndWait(toolbarElements.transposeButton, transposeElements.headerCheckbox.get(0));
        let rawArray: string[] = await transposeElements.headers.getText();
        let partName: string = rawArray[1];
        await workspaceLogic.addToWorkspacePartsFromTranspose();
        await meganav.goToFeatureWithMeganav(meganavItems.myWorkspace, gridElements.gridWrapper);
        await workspaceLogic.goToWorkspaceTab(linksNames.workspaceParts, gridElements.newGridCellByRowIndex(0).get(0));
        await expect(await partName).toContain(await gridElements.newGridCellByRowIndex(0).get(0).getText());
    });
