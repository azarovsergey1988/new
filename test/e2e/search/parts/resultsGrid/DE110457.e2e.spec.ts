...
describe('DE110457', () => {
    ...
    it('transpose pagination should show quantity of items that set in search settings ', async () => {
        ...
        await expect (await gridElements.currentPaginationValue.get(0).getAttribute('value')).toEqual('4: 500');
        // await expect (await gridElements.itemsCountInGrid.getText()).toContain('1 - 50');
        await Header.hoverOnHeaderItemAndClickOnDisplayedLink(headerElements.settingsIcon, headerItems.settingsMenuLinks[3],
            settings.module);
