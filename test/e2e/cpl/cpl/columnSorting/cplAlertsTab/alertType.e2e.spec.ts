...
describe('CPL ALerts tab- Alert Type - column filtering', () => {
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[1] + ' option', async () => {
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Alert Type',
            false);
        await grid.newGridOpenFilterBoxByName('Alert Type');
...
describe('CPL ALerts tab- Alert type - column sorting', () => {
    ...
    it('should be ASC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Alert Type', true);
        const activeNameActualAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Alert Type'])).filter(String);
    ...
    it('should be DESC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Alert Type', true);
        const activeNameActualDescValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Alert Type'])).filter(String);
    ...
    it('should be Clear sorting', async () => {
        ...
        const activeNameAscValues: string[] = (await grid.newGridReturnCellValuesByColumnName(1,
            'Alert Type')).filter(String);
        await grid.newGridOpenFilterBoxByName('Alert Type');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.clearSort, gridElements.newGridRows.get(0));
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Alert Type', false);
        const activeNameClearValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Alert Type'])).filter(String);
    ...
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(1, 'Alert Type');
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('Alert Type');
        await toolbar.unhideCellNameWithUnhideAll('Alert Type');
        await expect(await gridElements.newGridHeaderByName('Alert Type').isDisplayed()).toBeTruthy();
    });
});
