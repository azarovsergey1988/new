...
describe('CPL Alerts tab - LTB Date - column sorting', () => {
    ...
    it('should be sort options', async () => {
        ...
        await grid.newGridOpenFilterBoxByName('LTB Date');
    ...
    it('should be ASC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'LTB Date', true);
        const activeNameActualAscValues: any[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['LTB Date'])).map(Date.parse);
    ...
    it('should be DESC sorting by default', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'LTB Date', true);
        const activeNameActualDescValues: any[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['LTB Date'])).map(Date.parse);
    ...
    it('should be Clear sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'LTB Date', false);
        const activeNameClearValues: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['LTB Date']);
    ...
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(1, 'LTB Date');
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('LTB Date');
        await toolbar.unhideCellNameWithCellValue('LTB Date');
        await expect(await gridElements.newGridHeaderByName('LTB Date').isDisplayed()).toBeTruthy();
    });
});

describe('CPL Alerts tab - LTB Date - column filtering', () => {
    ...
    it('should filter with ' + columnHeaders.dateRange.dateOption[0] + ' option', async()=>{
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'LTB Date',
            false);
        await grid.newGridOpenFilterBoxByName('LTB Date');
        await grid.switchToSortColumnMenu();
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.ascSort, gridElements.newGridRows.get(0));
        await grid.newGridOpenFilterBoxByName('LTB Date');
        await grid.switchToFilterColumnMenu();
        const dateValues: any[] = await grid.newGridReturnCellValuesByColumnName(1,'LTB Date');
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'LTB Date',
            true);
        const datesValuesAfterDateFiltering: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['LTB Date']);
    ...
    it('should filter with ' + columnHeaders.dateRange.dateOption[1] + ' option', async()=> {
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'LTB Date',
            true);
        const dateValues: any[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['LTB Date']);
        await grid.newGridOpenFilterBoxByName('LTB Date');
        await Dropdown.openDropdownByClickOnElement(gridElements.columnsSort.dateRangedropdownButton);
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'LTB Date',
            true);
        const datesValuesAfterDateFiltering: any[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['LTB Date']);
        for (let i: number = 0; i < datesValuesAfterDateFiltering.length; i++) {
