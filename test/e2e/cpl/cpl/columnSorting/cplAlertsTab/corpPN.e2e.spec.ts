...
describe('CPL ALerts tab- Corp P/N - column filtering', () => {
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[1] + ' option', async () => {
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(0, 'Corp P/N',
            false);
        await grid.newGridOpenFilterBoxByName('Corp P/N');
...
describe('CPL ALerts tab- Corp P/N - column sorting', () => {
    ...
    it('should be ASC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 0,
            'Corp P/N', true);
        const activeNameActualAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Corp P/N'])).filter(String);
    ...
    it('should be DESC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 0,
            'Corp P/N', true);
        const activeNameActualDescValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Corp P/N'])).filter(String);
    ...
    it('should be Clear sorting', async () => {
        ...
        const activeNameAscValues: string[] = (await grid.newGridReturnCellValuesByColumnName(0,
            'Corp P/N')).filter(String);
        await grid.newGridOpenFilterBoxByName('Corp P/N');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.clearSort, gridElements.newGridRows.get(0));
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 0,
            'Corp P/N', false);
        const activeNameClearValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Corp P/N'])).filter(String);
    ...
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(0, 'Corp P/N');
    ...
    it('should be option Hide and Unhide column', async () => {
        ...
        await expect(await gridElements.newGridHeaderByName('Corp P/N').isDisplayed()).toBeTruthy();
    });
});
