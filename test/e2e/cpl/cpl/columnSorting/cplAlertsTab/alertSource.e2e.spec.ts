...
describe('CPL Alerts tab- Alert Source - column filtering', () => {
    ...
    beforeAll(async () => {
        ...
        await grid.newGridHideColumnsRange(['Corp P/N','Corp Name','Matched Mfr P/N','Matched Mfr Name',
            'Alert Type','Alert Description','Alert Comments','IHS Alert Date', 'LTB Date', 'LTD Date']);
    });

    it('should be validation rules for filtering', async () => {
        await grid.newGridOpenFilterBoxByName('Alert Source');
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[1] + ' option', async () => {
        const values: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Alert Source']);
        const sortValue: string = values[1].slice(0,2);
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Alert Source',
            false);
        await grid.newGridOpenFilterBoxByName('Alert Source');
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[2] + ' option', async () => {
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Alert Source',
            true);
        const psNameValues: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Alert Source']);
        await grid.newGridOpenFilterBoxByName('Alert Source');
...
describe('CPL Alerts tab- Alert Source - column sorting', () => {
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('Alert Source');
        await toolbar.unhideCellNameWithCellValue('Alert Source');
        await expect(await gridElements.newGridHeaderByName('Alert Source').isDisplayed()).toBeTruthy();
    });
});
