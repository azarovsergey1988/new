...
import {dateRange} from "../../../../../../elements/elements";
...
describe('CPL Alerts tab - Mfr Issued Date - column sorting', () => {
    ...
    it('should be sort options', async () => {
        ...
        await grid.newGridOpenFilterBoxByName('Mfr Issued Date');
    ...
    it('should be ASC sorting', async () => {
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Mfr Issued Date', false);
        await grid.newGridOpenFilterBoxByName('Mfr Issued Date');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.ascSort, gridElements.newGridRows.get(0));
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Mfr Issued Date', true);
        const activeNameActualAscValues: any[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Mfr Issued Date'])).map(Date.parse);
    ...
    it('should be DESC sorting by default', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Mfr Issued Date', true);
        const activeNameActualDescValues: any[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Mfr Issued Date'])).map(Date.parse);
    ...
    it('should be Clear sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Mfr Issued Date', false);
        const activeNameClearValues: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Mfr Issued Date']);
    ...
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(1, 'Mfr Issued Date');
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('Mfr Issued Date');
        await toolbar.unhideCellNameWithCellValue('Mfr Issued Date');
        await expect(await gridElements.newGridHeaderByName('Mfr Issued Date').isDisplayed()).toBeTruthy();
    });
});

describe('CPL Alerts tab - Mfr Issued Date - column filtering', () => {
    ...
    it('should filter with ' + columnHeaders.dateRange.dateOption[0] + ' option', async()=>{
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Mfr Issued Date',
            false);
        await grid.newGridOpenFilterBoxByName('Mfr Issued Date');
        await grid.switchToSortColumnMenu();
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.ascSort, gridElements.newGridRows.get(0));
        await grid.newGridOpenFilterBoxByName('Mfr Issued Date');
        await grid.switchToFilterColumnMenu();
        const dateValues: any[] = await grid.newGridReturnCellValuesByColumnName(1, 'Mfr Issued Date');
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Mfr Issued Date',
            true);
        const datesValuesAfterDateFiltering: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Mfr Issued Date']);
    ...
    it('should filter with ' + columnHeaders.dateRange.dateOption[1] + ' option', async()=> {
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Mfr Issued Date',
            true);
        const dateValues: any[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Mfr Issued Date']);
        await grid.newGridOpenFilterBoxByName('Mfr Issued Date');
        await Dropdown.openDropdownByClickOnElement(gridElements.columnsSort.dateRangedropdownButton);
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Mfr Issued Date',
            true);
        const datesValuesAfterDateFiltering: any[] = await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Mfr Issued Date']);
        for (let i: number = 0; i < datesValuesAfterDateFiltering.length; i++) {
