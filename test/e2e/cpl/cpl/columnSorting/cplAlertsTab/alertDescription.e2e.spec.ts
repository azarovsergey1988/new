...
describe('CPL Alerts tab- Alert Description - column filtering', () => {
    ...
    it('should be validation rules for filtering', async () => {
        await grid.newGridOpenFilterBoxByName('Alert Description');
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[1] + ' option', async () => {
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Alert Description',
            false);
        await grid.newGridOpenFilterBoxByName('Alert Description');
...
describe('CPL Alerts tab- Alert Description - column sorting', () => {
    ...
    it('should be ASC sorting', async () => {
        ...
        const activeNameActualAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Alert Description'])).filter(String);
    ...
    it('should be DESC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Alert Description', true);
        const activeNameActualDescValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Alert Description'])).filter(String);
    ...
    it('should be Clear sorting', async () => {
        ...
        const activeNameAscValues: string[] = (await grid.newGridReturnCellValuesByColumnName(1,
            'Alert Description')).filter(String);
        await grid.newGridOpenFilterBoxByName('Alert Description');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.clearSort, gridElements.newGridRows.get(0));
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Alert Description', false);
        const activeNameClearValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.cplAlertstab['Alert Description'])).filter(String);
    ...
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(1, 'Alert Description');
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('Alert Description');
        await toolbar.unhideCellNameWithUnhideAll('Alert Description');
        await expect(await gridElements.newGridHeaderByName('Alert Description').isDisplayed()).toBeTruthy();
    });
});
