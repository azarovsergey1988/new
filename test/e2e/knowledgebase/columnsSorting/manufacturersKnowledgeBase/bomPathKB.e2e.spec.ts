...
describe('Knowledge Base - Manufactures Knowledge Base - BOM Path/ Column filters', () => {

    beforeAll(async () => {
        await login.loginWithDirectLink(browser.params.groupAdminUrl);
        await meganav.goToFeatureWithMeganavSubLinks(meganavItems.knowledgeBase,
            meganavItems.knowledgeBaseSubItems.mfrKnowledgeBase, gridElements.newGridCellWithoutContentByRowIndex(0).get(1));
        await grid.newGridHideColumnsRange([ 'Exception Mfr Name','Accepted Mfr Name']);
    });
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[0] + ' option', async () => {
        const values: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['BOM Path'])).filter(String);
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'BOM Path',
            true);
        const afterSortValues: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['BOM Path']);
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[2] + ' option', async () => {
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'BOM Path',
            true);
        const psNameValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['BOM Path'])).filter(String);
        await grid.newGridOpenFilterBoxByName('BOM Path');
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'BOM Path',
            true);
        const psValuesAfterSorting: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['BOM Path']);
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[1] + ' option', async () => {
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'BOM Path',
            true);
        const bomNameValuesAfterSorting: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['BOM Path']);
...
describe('Knowledge Base - Manufactures Knowledge Base - BOM Path/ Column sorting', () => {
    ...
    it('should be ASC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'BOM Path', true);
        const activeNameActualAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['BOM Path'])).filter(String);
    ...
    it('should be DESC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'BOM Path', true);
        const activeNameActualDescValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['BOM Path'])).filter(String);
    ...
    it('should be Clear sorting', async () => {
        await grid.newGridOpenFilterBoxByName('BOM Path');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.ascSort, gridElements.newGridRows.get(0));
        const activeNameAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['BOM Path'])).filter(String);
        await grid.newGridOpenFilterBoxByName('BOM Path');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.clearSort, gridElements.newGridRows.get(0));
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'BOM Path', false);
        const activeNameClearValues: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['BOM Path']);
