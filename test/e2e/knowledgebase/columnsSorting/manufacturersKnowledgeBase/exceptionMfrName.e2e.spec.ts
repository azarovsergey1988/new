...
describe('Knowledge Base - Manufacturers Knowledge Base - Exception Mfr Name/ Column filters', () => {
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[2] + ' option', async () => {
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(0, 'Exception Mfr Name',
            false);
    ...
    it('should work Clear Filter button', async () => {
        const psNameValues: string[] = await grid.newGridReturnCellValuesByColumnName(0,
            'Exception Mfr Name');
        const sortValue: string = psNameValues[1].slice(0, 2);
        await grid.newGridOpenFilterBoxByName('Exception Mfr Name');
        await Actions.sendKeys(gridElements.columnsSort.input, sortValue);
        await grid.peformSearchInColumnSort(gridElements.newGridRows.get(0));
        await grid.newGridOpenFilterBoxByName('Exception Mfr Name');
...
describe('Knowledge Base - Manufacturers Knowledge Base - Exception Mfr Name/ Column sorting', () => {
    ...
    it('should be ASC sorting', async () => {
        ...
        const activeNameActualAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['Exception Mfr Name'])).filter(String);
    ...
    it('should be DESC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 0,
            'Exception Mfr Name', true);
        const activeNameActualDescValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.mfrsKnowledgeBaseColumnNamesContent['Exception Mfr Name'])).filter(String);
    ...
    it('should be Clear sorting', async () => {
        ...
        const activeNameAscValues: string[] = (await grid.newGridReturnCellValuesByColumnName(0,
            'Exception Mfr Name')).filter(String);
        await grid.newGridOpenFilterBoxByName('Exception Mfr Name');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.clearSort, gridElements.newGridRows.get(0));
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 0,
            'Exception Mfr Name', false);
        const activeNameClearValues: string[] = (await grid.newGridReturnCellValuesByColumnName(0,
            'Exception Mfr Name')).filter(String);
    ...
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(0, 'Exception Mfr Name');
        await expect(await JsScripts.returnElementWidthByCssAndNumber(gridElements.newGridHeaderCss,
            colNumber+1)).toEqual(245);
    });
