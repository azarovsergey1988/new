...
describe('Knowledge Base - Parts Knowledge Base - Exception Mfr PN/ Column filters', () => {
    ...
    it('should be validation rules for filtering', async () => {
        await grid.newGridOpenFilterBoxByName('Exception Mfr P/N');
    ...
    it('should filter with ' + columnHeaders.bom.filterOptions[2] + ' option', async () => {
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(0, 'Exception Mfr P/N',
            false);
...
describe('Knowledge Base - Parts Knowledge Base - Exception Mfr P/N/ Column sorting', () => {
    ...
    it('should be ASC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 0,
            'Exception Mfr P/N', false);
        await grid.newGridOpenFilterBoxByName('Exception Mfr P/N');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.ascSort, gridElements.newGridRows.get(0));
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 0,
            'Exception Mfr P/N', true);
        const activeNameActualAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Exception Mfr P/N'])).filter(String);
    ...
    it('should be DESC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 0,
            'Exception Mfr P/N', true);
        const activeNameActualDescValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Exception Mfr P/N'])).filter(String);
    ...
    it('should be Clear sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 0,
            'Exception Mfr P/N', false);
        const activeNameClearValues: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Exception Mfr P/N']);
    ...
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(0, 'Exception Mfr P/N');
        await expect(await JsScripts.returnElementWidthByCssAndNumber(gridElements.newGridHeaderCss,
            colNumber+1)).toEqual(176);
    });

    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('Exception Mfr P/N');
        await toolbar.unhideCellNameWithUnhideAll('Exception Mfr P/N');
        await expect(await gridElements.newGridHeaderByName('Exception Mfr P/N').isDisplayed()).toBeTruthy();
    });
});