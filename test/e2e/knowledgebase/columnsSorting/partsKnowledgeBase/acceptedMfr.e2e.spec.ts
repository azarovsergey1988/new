...
import {commonElements, gridElements, partStandardization} from "../../../../../elements/elements";
...
describe('Knowledge Base - Parts Knowledge Base - Accepted Mfr/ Column sorting', () => {
    ...
    it('should be sort options', async () => {
        await grid.newGridOpenFilterBoxByName('Accepted Mfr');
    ...
    it('should be ASC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Accepted Mfr', true);
        const activeNameActualAscValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Accepted Mfr'])).filter(String);
    ...
    it('should be DESC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Accepted Mfr', true);
        const activeNameActualDescValues: string[] = (await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Accepted Mfr'])).filter(String);
    ...
    it('should be Clear sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Accepted Mfr', false);
        const activeNameClearValues: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Accepted Mfr']);
...
// fail until the bug is fixed
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(1, 'Accepted Mfr');
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('Accepted Mfr');
        await toolbar.unhideCellNameWithUnhideAll('Accepted Mfr');
        await expect(await gridElements.newGridHeaderByName('Accepted Mfr').isDisplayed()).toBeTruthy();
    });
});

describe('Knowledge Base - Parts Knowledge Base - Accepted Mfr/ Column filter', () => {
    ...
    it('should be validation rules for filtering', async () => {
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Accepted Mfr',
            false);
        const values: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Accepted Mfr']);
        const sortValue: string = await values[1].slice(0,7);
        await grid.newGridOpenFilterBoxByName('Accepted Mfr');
    ...
    it('should filter with type ahead option', async () => {
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Accepted Mfr',
            false);
        const values: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Accepted Mfr']);
        const sortValue: string = values[1].slice(0, 7);
        await grid.newGridOpenFilterBoxByName('Accepted Mfr');
        await typeAhead.typeAheadChecking(gridElements.columnsSort.mfrTypeAhead, sortValue);
        await grid.peformSearchInColumnSort(gridElements.newGridRows.last());
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Accepted Mfr',
            true);
        const mfrNameValuesAfterSorting: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Accepted Mfr']);
    ...
    it('should work Clear Filter button', async () => {
        ...
        await grid.newGridCheckFilterIconPresentInColumnHeaderByColumnName(1, 'Accepted Mfr',
            false);
        const mfrNameValuesAfterClearing: string[] = await grid.getCellValuesByCellId(columnIdByColumnName.partsKnowledgeBaseColumnNamesContent['Accepted Mfr']);
