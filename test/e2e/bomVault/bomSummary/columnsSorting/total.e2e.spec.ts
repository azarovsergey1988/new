...
describe('BOM Summary - Total column', () => {
    ...
    it('should be ASC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Total', true);
        const totalActualAscValues: string[] = await grid.newGridReturnCellValuesByColumnName(1,
            'Total');
        const totalExpectAscValues: string[] = await grid.compareAscNumberValues(totalActualAscValues.slice());
        await expect(totalExpectAscValues).toEqual(totalActualAscValues);
    });
    ...
    it('should be DESC sorting', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Total', true);
        const totalActualDescValues: string[] = await grid.newGridReturnCellValuesByColumnName(1,
            'Total');
        const totalExpectDescValues: string[] = await grid.compareDescNumberValues(totalActualDescValues.slice());
        await expect(totalExpectDescValues).toEqual(totalActualDescValues);
    });

    it('should be Clear sorting', async () => {
        await grid.newGridOpenFilterBoxByName('Total');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.ascSort, gridElements.newGridRows.get(0));
        const totalAscValues: string[] = await grid.newGridReturnCellValuesByColumnName(1,
            'Total');
        await grid.newGridOpenFilterBoxByName('Total');
        await grid.selectOptionInColumnFilter(columnHeaders.commonColumnSortOptions.clearSort, gridElements.newGridRows.get(0));
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Total', false);
        const totalClearValues: string[] = await grid.newGridReturnCellValuesByColumnName(1,
            'Total');
        await expect(totalAscValues).not.toEqual(totalClearValues);
    });
