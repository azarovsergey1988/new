...
describe('BOM Vault - BOM Name column', () => {
...
describe('BOM Vault - BOM Name column filter', () => {
    ...
    it('should sorting with ' + columnHeaders.bom.filterOptions[0] + ' option', async () => {
        ...
        await Actions.sendKeys(gridElements.columnsSort.input, 'Reg');
        ...
        for (let i: number = 0; i < bomNameValues.length; i++) {
            await expect(bomNameValues[i].toLowerCase()).toContain('reg');
        }
    });
