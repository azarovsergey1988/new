...
describe('View Single BOM /Match Parts tab - Part Description/ Column sorting', () => {
    ...
    it('should be sort options', async () => {
        await grid.newGridOpenFilterBoxByName('Part Description');
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('Part Description');
        await toolbar.unhideCellNameWithCellValue('Part Description');
        await expect(await gridElements.newGridHeaderByName('Part Description').isDisplayed()).toBeTruthy();
    });
    ...
    it('should work Reset columns', async () => {
        ...
        await grid.newGridHideColumnsRange([ 'Imported Part Number', 'Matched Part Number','Imported Manufacturer Name', 'Matched Manufacturer Name','Imported Part Description', 'Internal Part Number'])
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(1, 'Part Description');
        await expect(await JsScripts.returnElementWidthByCssAndNumber(gridElements.newGridHeaderCss,
            colNumber+1)).toEqual(120);
    });
