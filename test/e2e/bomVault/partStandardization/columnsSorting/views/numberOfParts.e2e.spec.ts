...
describe('Part Standardisation - Views Tab - # of Parts column', () => {
    ...
    it('should be sort options', async () => {
        await grid.newGridOpenFilterBoxByName('# of Parts');
    ...
    it('should be ASC sorting', async () => {
        ...
        const activeNameActualAscValues: string[] = await grid.replaceNumberValues((
            await grid.newGridReturnCellValuesByColumnName(1, '# of Parts')).filter(Number));
    ...
    it('should work Reset columns', async () => {
        ...
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(1, '# of Parts');
        await expect(await JsScripts.returnElementWidthByCssAndNumber(gridElements.newGridHeaderCss,
            colNumber+1 )).toEqual(92);
    });

    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('# of Parts');
        await toolbar.unhideCellNameWithUnhideAll('# of Parts');
        await expect(await gridElements.newGridHeaderByName('# of Parts').isDisplayed()).toBeTruthy();
    });
});