...
describe('Part Standardisation - Summary Tab - Part Type column sort', ()=> {
    beforeAll(async() => {
        await login.loginWithDirectLink(browser.params.groupAdminUrl);
        await meganav.goToFeatureWithMeganavSubLinks(meganavItems.boms,
            meganavItems.bomsSubItems.partStandardization, gridElements.gridWrapper);
        await partStandardizationLogic.createNewReprocessedView('partType', 6);
    });
    ...
    afterAll(async() =>{
        ...
        await meganav.goToFeatureWithMeganavSubLinks(meganavItems.boms,
            meganavItems.bomsSubItems.partStandardization, gridElements.gridWrapper);
        await partStandardizationLogic.deleteViewByName('partType');
    });
    ...
    beforeEach(async() => {
        ...
        await grid.mechanismCheckCheckboxByName('Name', 'partType');
    ...
    it('should has default width', async () => {
        const colNumber: number = await grid.newGridReturnColumnNumberByColumnName(1, 'Part Type');
    ...
    it('should be sort option', async () => {
        await grid.newGridOpenFilterBoxByName('Part Type');
    ...
    it('should be ASC sort', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Part Type', true);
        const mfrNameActualAscValues: string[] = (await partStandardizationLogic.returnCellValuesByColumnNameSummaryTab('Part Type')).filter(String);
    ...
    it('should be DESC sort', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('DESC', 1,
            'Part Type', true);
        const mfrNameActualDescValues: string[] = (await partStandardizationLogic.returnCellValuesByColumnNameSummaryTab('Part Type')).filter(String);
    ...
    it('should be Clear sort', async () => {
        ...
        await grid.newGridCheckSortingIconPresentInColumnHeaderByColumnName('ASC', 1,
            'Part Type', false);
        await expect(await gridElements.ascSortHeaderIconByName('Part Type').isDisplayed()).toBeFalsy();
        await expect(await gridElements.descSortHeaderIconByName('Part Type').isDisplayed()).toBeFalsy();
    });
    ...
    it('should be option Hide and Unhide column', async () => {
        await grid.newGridHideColumnByName('Part Type');
