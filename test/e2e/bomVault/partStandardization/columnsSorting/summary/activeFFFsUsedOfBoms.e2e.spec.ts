...
describe('Part Standardisation - Summary Tab - Active FFFs column sort', ()=> {
    beforeAll(async() => {
        ...
        await partStandardizationLogic.createNewReprocessedView('activeFFSUsed', 6);
    });
    ...
    afterAll(async() =>{
        ...
        await meganav.goToFeatureWithMeganavSubLinks(meganavItems.boms,
            meganavItems.bomsSubItems.partStandardization, gridElements.gridWrapper);
        await partStandardizationLogic.deleteViewByName('activeFFSUsed');
    });
    ...
    beforeEach(async() => {
        ...
        await grid.mechanismCheckCheckboxByName('Name', 'activeFFSUsed');
