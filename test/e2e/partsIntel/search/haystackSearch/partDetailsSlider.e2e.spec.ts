...
describe('Haystack Part Details Slider ', () => {
    ...
    xit('should be column headers and attributes for Segment A', async () => {
        await grid.checkingColumnHeaders(0, columnHeaders.commonAttributeValue);
        await expect(await searchElements.haystack.sliderAttributes.getText()).toEqual(haystackSearchConst.segmentAAtrributes)
    });


    xit('should switch to MCRL and be column headers', async () => {
   ...
   xit('should switch to Parametric tab and be column headers', async () => {
        await Slider.switchOnTheLeftNavSectionByClickingOnTheSectionName(haystackSearchConst.parametric);
        await grid.checkingColumnHeaders(0, haystackSearchConst.parametricColumnHeaders);
    });
