...
describe('Part Details modal with links from BOM Tree Parts grid  - Matched Part Number', () => {

    it('should open Part details modal by clicking on part number - BOM Tree Parts', async () => {
    ...
    xit(" should be export dropdown - Matched Part Number - BOM Tree Parts", async () => {
        await partDetailsLogic.exportDropdown();
    });

    it('should be active in the left nav - Matched Part Number BOM Tree Parts ', async () => {
        await expect(await partDetailsElements.activeLeftNav.getText()).toEqual(partDetailsData.leftNav.activePartDetails);
    });

    it('should be basic section in the left nav - Matched Part Number BOM Tree Parts ', async () => {
        await partDetailsLogic.checkBasicSection()
    });

    it('should be Life Cycle section in the left nav - Matched Part Number BOM Tree Parts ', async () => {
        await partDetailsLogic.checkLifeCycleSection()
    });

    it('should be Compliance section in the left nav - Matched Part Number BOM Tree Parts ', async () => {
        await partDetailsLogic.checkComplianceSection()
    });

    it('should be Attribute and Value column headers -  Matched Part Number - BOM Tree Parts', async () => {
    ...
    it('should not be sorting for Attribute column headers - Matched Part Number - BOM Tree Parts',  async () => {
        await grid.notBeSortingPartDetails();
    });

    it('should be add button with dropdown list  - Matched Part Number - BOM Tree Parts',  async () => {
        await partDetailsLogic.addDropdown();
    });

    it('should open Add to BOM(s) modal  - Matched Part Number - BOM Tree Parts',  async () => {
        await partDetailsLogic.addToBomModal();
    });

    it('should open view alternates modal  - Matched Part Number - BOM Tree Parts',  async () => {
    ...
    it('should open  print preview all modal - Matched Part Number -  BOM Tree Parts', async () => {
        await partDetailsLogic.printModal();
    });

    it('should open specific print preview modals - Matched Part Number - BOM Tree Parts', async () => {
        await partDetailsLogic.checkingPrintPreviewModals();
    });

    it('should go to parametric by clicking on the search by example - Matched Part Number - BOM Tree Parts', async () => {
